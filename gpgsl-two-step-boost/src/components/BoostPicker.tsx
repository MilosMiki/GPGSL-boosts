    ...
    const fetchData = async () => {
      try {
        ...
        const foundDriver = fetchedDrivers.find(
          d => d.username.localeCompare(username, undefined, { sensitivity: 'base' }) === 0
        );
        ...
        const foundTeam = fetchedTeams.find(
          t => t.username.localeCompare(username, undefined, { sensitivity: 'base' }) === 0
        );
  ...
  return (
    <div className="boost-picker">
      <div className="boost-options">
        <div 
          className={`boost-option ${driverBoost ? 'selected' : ''} ${!driver ? 'disabled' : ''}`}
        ...
        <div 
          className={`boost-option ${teamBoost ? 'selected' : ''} ${!team ? 'disabled' : ''}`}
